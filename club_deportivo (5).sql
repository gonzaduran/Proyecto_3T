...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 26-05-2025 a las 11:12:35
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `asistenciasPorActividad` (IN `actividadDeseada` VARCHAR(100))   BEGIN
  SELECT 
    A.id_asistencia,
    S.nombre AS nombre_socio,
    S.apellido_1,
    AC.nomAct AS actividad,
    A.fecha,
    A.estado
  FROM asistencias A
  JOIN socios S ON A.id_socio = S.id_socio
  JOIN actividades AC ON A.id_actividad = AC.idAct
  WHERE AC.nomAct = actividadDeseada;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listactbyzona` (IN `zonadeseada` VARCHAR(50))   BEGIN
  SELECT 
    idAct, 
    A.nomAct,
    A.horario,
    ...
    E.especialidad
  FROM Actividades A 
  JOIN Zonas Z ON A.idZona = Z.idZona 
  JOIN Entrenadores E ON A.idEntre = E.id_entrenador
  WHERE Z.nomZona = zonadeseada;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `marcarPagoRealizado` (IN `socioID` INT)   BEGIN
  UPDATE pagos
  SET pagado = 'Pagado'
...
('gonzalo', 20, 'durán', 'parreño', 106, 'activo', NULL, NULL);
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `baja_temporal` BEFORE DELETE ON `socios` FOR EACH ROW BEGIN
  IF OLD.estado = 'activo' THEN
    -- En lugar de borrar, actualizamos el estado a 'inactivo'
    UPDATE socios
    SET estado = 'inactivo'
    WHERE id_socio = OLD.id_socio;

    -- Cancelamos el borrado lanzando un error (para evitar que lo elimine)
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Baja temporal aplicada, el socio no ha sido eliminado.';
  END IF;
END
$$
DELIMITER ;
...
--
-- AUTO_INCREMENT de la tabla `horarios_entrenador`
--
ALTER TABLE `horarios_entrenador`
...
--
ALTER TABLE `socios`
  MODIFY `id_socio` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=107;
